...
const { sendOrderShipped, sendOrderDelivered } = require("../middleware/mailersend");
    ...
    const updatedOrder = await Order.findByIdAndUpdate(
      ...
      { new: true }
		...
		if(req.body.delivery_status === "dispatched") {
			sendOrderShipped(req.body.shipping?.email, "Uw bestelling is Onderweg!", req.body)
		} else if (req.body.delivery_status === "delivered") {
			sendOrderDelivered(req.body.shipping?.email, "Uw bestelling is Thuisbezorgd!", req.body)
		}
...
router.delete("/:id", isAdmin, async (req, res) => {
  try {
    ...
    res.status(200).send("De bestelling is verwijderd...");
  } catch (err) {
    res.status(500).send(err);
  }
});
